...
namespace AthiVegam::Graphics
{
		...
		, m_ebo(0)
	{
		...
		VEGAM_CHECK_GL_ERROR;
		{
			...
			VEGAM_CHECK_GL_ERROR;
			{
				glBufferData(GL_ARRAY_BUFFER, static_cast<uint64_t>(vertexCount) * dimensions * sizeof(float), vertexArray, GL_STATIC_DRAW); 
				VEGAM_CHECK_GL_ERROR;
		...
		: Mesh(vertexArray, vertexCount, dimensions)
	{
		...
		VEGAM_CHECK_GL_ERROR;
		{
			...
			VEGAM_CHECK_GL_ERROR;
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<uint64_t>(elementCount) * sizeof(uint32_t), elementArray, GL_STATIC_DRAW); 
			VEGAM_CHECK_GL_ERROR;
		}
		glBindVertexArray(0); 
		VEGAM_CHECK_GL_ERROR;
